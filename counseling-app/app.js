...
function updateCarePrices() {
    const baseCarePrice = state.selectedCourse?.type === 'light' ? 1000 : 0;
    ...
    // ライトコースの場合は次へボタンを有効化（ケア剤選択なしでもOK）
    if (state.selectedCourse?.type === 'light') {
...
function updateSummary() {
    ...
    if (state.selectedCare) {
        ...
        const carePrice = state.selectedCourse?.type === 'light' ? 1000 : 0;
